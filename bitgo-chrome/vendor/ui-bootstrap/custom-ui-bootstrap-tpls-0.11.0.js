  ...
  .factory('$modalStack', ['$transition', '$timeout', '$document', '$compile', '$rootScope', '$$stackedMap',
    function ($transition, $timeout, $document, $compile, $rootScope, $$stackedMap) {
      ...
      $modalStack.close = function (modalInstance, result) {
        var modalWindow = openedWindows.get(modalInstance) && openedWindows.get(modalInstance).value;
        if (modalWindow) {
 ...
 */
  ...
  return {
    ...
    link:function (originalScope, element, attrs, modelCtrl) {
      ...
      $document.bind('mousedown', dismissClickHandler);
    ...
    return {
      ...
      link:function (scope, element, attrs) {
        ...
        scope.isOpen = function () {
          // emits only when it changes state from close to open or vice versa
          if (isClosed && scope.matches.length > 0){
            scope.$emit('typeaheadPopup.isClosed', false);
            isClosed = false;
          }
          else if (!isClosed && !(scope.matches.length > 0)) {
            scope.$emit('typeaheadPopup.isClosed', true);
            isClosed = true;
          }
  ...
  .filter('typeaheadHighlight', function() {
    ...
    return function(matchItem, query) {
      ...
      return query ? ('' + matchItem).replace(new RegExp(escapeRegexp(query), 'gi'), '<b>$&</b>') : matchItem;
    };
  });
...
angular.module("template/typeahead/typeahead-popup.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/typeahead/typeahead-popup.html",
    // BG Modified
    "<ul class=\"dropdown-menu autoSuggest\" ng-style=\"{display: isOpen()&&'block' || 'none', top: position.top+'px', left: position.left+'px'}\" style=\"display: block;\" role=\"listbox\" aria-hidden=\"{{!isOpen()}}\">\n" +
    "    <li class=\"autoSuggest-item\" ng-repeat=\"match in matches track by $index\" ng-class=\"{ 'autoSuggest-item--is-selected' : isActive($index) }\" ng-mouseenter=\"selectActive($index)\" ng-mousedown=\"$event.stopPropagation();\" ng-click=\"selectMatch($index); $event.stopPropagation();\" bg-typeahead-trigger role=\"option\" id=\"{{match.id}}\">\n" +
